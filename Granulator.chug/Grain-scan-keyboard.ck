...
for( int i; i < del.size(); i++ ) del[i].set( (i+1)*48::ms, (1+(i+1)*48)::ms );

for( int i; i < test.size(); i++ )
{
    test[i].pitch( 1.0 );
    test[i].size( 10.0 );
    test[i].randomPosition( 0.0 );
}
...
fun void update( Matrix4 object )
{
    ...
    while( true )
    {
        ...
        for( int i; i < 4; i++ ) 
        {
            for( int j; j < 4; j++ )
            {
                if( j == i ) object.entry( 0.5 * Math.cos( mult * phase ), i, j );
                else object.entry( 0.5 * Math.sin( mult * phase ), i, j );
            }
        }
        10::ms => now;
    }
    ...
    while( key.recv( msg ) )
    {
        if( msg.isButtonDown() )
        {
            ...
            else if ( msg.key == 6 )
            {
                ...
                {
                    // test[i].randomSize( Math.random2f( 0, 2000 ) );
                    test[i].randomPosition( Math.clampf( test[i].randomPosition() + ( test[i].randomPosition() * 0.1 ) + 0.001 , 0, 12000.0 ) );
                    <<< "+ randomness of grain position ", test[i].randomPosition() >>>;
                }
            }
